...
public class UsuarioServiceREST {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public Response login(Tbl_Usuario usuario) {
        try {
            boolean valid = usuarioService.login(usuario.getUsername(), usuario.getPassword());

            if (valid) {
                return Response.ok()
                        .entity("{\"mensaje\": \"Login exitoso\", \"usuario\": \"" + usuario.getUsername() + "\"}")
                        .build();
            } else {
