...
public class EspecialidadServiceREST {
	...
	@Path("/Existe/{nombre}")
	@Produces(MediaType.APPLICATION_JSON)
	public boolean existeEspecialidad(@PathParam("nombre") String nombre) {
	    
	    return crud.ExisteEspecialidad(nombre);
	}
	@GET
	...
	public Response BuscarEspecialidadPorEspecialidad(@PathParam("nomespe") String nomespe) {
	    ...
	    if (resultado.getEspecialidad() == null) { // si no encontró registro
	    	return Response.status(Status.NOT_FOUND)
	    			.entity("{\"mensaje\":\"No se encontró Especialidad :" + nomespe+ "\"}")
	    			.build();
	    }
...
/*
		...
		@Produces(MediaType.APPLICATION_JSON)
		public Response buscarEspecialidadId(@PathParam("id")int id){
			Tbl_Especialidades espe = new Tbl_Especialidades();
		    espe.setIdEspecialidad(id);;
		    Tbl_Especialidades resultado = crud.buscarEspecialidadId(espe);
		    if (resultado.getEspecialidad() == null) { // si no encontró registro
		    	return Response.status(Status.NOT_FOUND)
		    			.entity("{\"mensaje\":\"No se encontró Especialidad con ID:" + id + "\"}")
		    			.build();
		    }
